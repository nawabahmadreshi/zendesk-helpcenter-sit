  ...
  function getWidgetCSS() {
    return `
      ...
      * {
      ...
      .aq-help-context-header { padding: 24px; background: linear-gradient(180deg, rgba(37, 99, 235, 0.05), transparent); border-bottom: 1px solid rgba(0, 0, 0, 0.05); }
  ...
  function initWidget() {
    ...
    async function runProactiveAnalysis(source = 'initial') {
        ...
        try {
            ...
            // Auto-open if configured (one-time per load) - Speed Up to 500ms
            if (source === 'initial' && !isOpen) {
                setTimeout(() => {
                    isOpen = true;
